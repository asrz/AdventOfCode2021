...
with open('input.in', 'r') as input_file:
    numbers = [line[:-1] for line in input_file.readlines()]

    max_mag = 0

    for combo in itertools.permutations(range(len(numbers)), 2):
        left = numbers[combo[0]]
        right = numbers[combo[1]]
        mag = magnitude(add(left, right))
